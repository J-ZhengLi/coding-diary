...
use std::{fmt, default, io::{self, Write}};
use rng_helper::random;

// example database for items with different rarity
static LEGENDARY_ITEMS: [&str; 15] = [
    "Mona", "Diluc", "Klee", "Jean", "Venti", "Albedo",
    "Keqing", "Xiao", "Qiqi", "Zhongli", "Ganyu",
    "Raiden Shogun", "Yomiya", "Itto", "Kamisato Ayaka"
];
static EPIC_ITEMS: [&str; 20] = [
    "Barbara", "Amber", "Bennet", "Sucross", "Razor",
    "Xiangling", "Xingqiu", "Chongyun", "Fischle", "Xinyan",
    "Yanfei", "Ningguang",
    "Black Sword", "The Flute", "Stringless", "Rust", "Prototype Sword",
    "Prototype Great Sword", "Prototype Bow", "Prototype Spear"
];
static RARE_ITEMS: [&str; 15] = [
    "Black Steel Set - Head", "Black Steel Set - Body", "Black Steel Set - Torso",
    "Black Steel Set - Foot", "Black Steel Set - Cape", "Black Steel Sword",
    "Black Steel Shield", "Black Steel Sword", "Ruby Necklace - ATK%", "Sapphire Ring - MAGIC%",
    "Moonlight Wand", "The Invader", "Debate Club", "Alsa's Cape", "Staff of Walnut" 
];
static SPECIAL_ITEMS: [&str; 15] = [
    "Silver Sword", "Silver Helmet", "Silver Armor", "Silver Shield", "Silver Boots",
    "Steel Sword", "Steel Shield", "Silver Helmet", "Silver Armor", "Silver Boots",
    "Wand of Gandoff", "Hat of Agatha", "Cape of Agatha", "Boots of Agatha", "The Charger"
];
static NORMAL_ITEMS: [&str; 10] = [
    "Sword", "Wooden Staff", "Magic Wand", "Bow", "Free Lance",
    "Leather Coat", "Leather Pants", "Leather Shoes", "Leather Hat", "Wedding Ring"
];
...
impl Default for Item {
    fn default() -> Item {
        Item { name: String::new(), rarity: Rarity::Normal }
    }
}
impl Item {
    fn show(&self) -> String {
...
fn pull(count: u8) -> (Vec<Item>, u8) {
    ...
    for _ in 0..count {
        ...
        //println!("{}", item_roll_num);
        if item_roll_num < MAX_OF_LEGENDARY {
            item.rarity = Rarity::Legendary;
            item.name = random::element_in_slice(&LEGENDARY_ITEMS).unwrap().to_string();
            ...
            item.name = random::element_in_slice(&SPECIAL_ITEMS).unwrap().to_string();
        } else {
...
-----------------------------------------------------------------------------",
    ...
    'game: loop {
        ...
        io::stdin().read_line(&mut input).expect("Fail to read input, try again.");
